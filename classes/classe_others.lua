...
function atributo_misc:DeadAtualizarBarra (morte, qual_barra, colocacao, instancia)
	...
	if (instancia.row_info.use_spec_icons) then
		...
		if (spec) then
			...
			local specCoords = spec and _detalhes.class_specs_coords[spec]

			if specCoords then
				esta_barra.icone_classe:SetTexCoord (_unpack (specCoords))
			else
				local classCoords = _detalhes.class_coords[self.classe or "UNKNOW"]
				if classCoords then
					esta_barra.icone_classe:SetTexCoord (_unpack (classCoords))
				end
			end
		else
