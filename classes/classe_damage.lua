...
-- Grimfall classless compatibility:
-- cache spell-name -> class mappings once instead of rebuilding every bar update.
local grimfallSpellNameClasses = nil

local function BuildGrimfallSpellNameClassCache()

	if grimfallSpellNameClasses then
		return grimfallSpellNameClasses
	end

	grimfallSpellNameClasses = {}

	if _detalhes.ClassSpellList then
		for knownSpellId, knownClass in pairs(_detalhes.ClassSpellList) do
			local knownName = GetSpellInfo(knownSpellId)

			if knownName and not grimfallSpellNameClasses[knownName] then
				grimfallSpellNameClasses[knownName] = knownClass
			end
		end
	end
...
local function GetDominantDamageClass(actor)

	if not actor or not actor.spells or not actor.spells._ActorTable then
		return actor and actor.classe or "UNKNOW"
	end
	...
	local spellNameClasses = BuildGrimfallSpellNameClassCache()

	for spellId, spellObject in pairs(actor.spells._ActorTable) do

		local numericSpellId = tonumber(spellId)

		-- 1 = melee, 2 = auto shot.
		-- Do not let generic auto attacks decide the visual class.
		if numericSpellId and numericSpellId > 2 then

			local class = nil

			-- exact spell ID first
			if _detalhes.ClassSpellList then
				class = _detalhes.ClassSpellList[numericSpellId]
			end

			-- Grimfall can use different/custom spell IDs,
			-- so fall back to matching by spell name.
			if not class then
				local spellName = GetSpellInfo(numericSpellId)

				if spellName then
					class = spellNameClasses[spellName]
				end
			end

			if class and spellObject and spellObject.total then
				classDamage[class] = (classDamage[class] or 0) + spellObject.total

				if classDamage[class] > bestDamage then
					bestDamage = classDamage[class]
					bestClass = class
				end
			end
		end
	...
	return bestClass or actor.classe or "UNKNOW"
end

function _detalhes:GetBarColor (actor)
	...
	elseif (actor.arena_team and _detalhes.color_by_arena_team) then
		...
		local displayClass = GetDominantDamageClass(actor)

		if (not is_player_class [displayClass] and actor.flag_original and _bit_band (actor.flag_original, 0x00000020) ~= 0) then
			return _unpack (_detalhes.class_colors.NEUTRAL)
		else
			return _unpack (_detalhes.class_colors [displayClass] or _detalhes.class_colors.UNKNOW)
		end
	end
end
...
function atributo_damage:AtualizaBarra (instancia, barras_container, qual_barra, lugar, total, sub_atributo, forcar, keyName, combat_time, percentage_type, use_animations, bars_show_data, bars_brackets, bars_separator)
	...
	-- Grimfall classless compatibility:
	-- dynamically refresh the bar whenever the dominant damage class changes.
	local dominantClass = GetDominantDamageClass(self)

	if esta_barra.grimfall_dominant_class ~= dominantClass then
		esta_barra.grimfall_dominant_class = dominantClass
		forcar = true
	end
...
--[[ exported]] function _detalhes:SetClassIcon (texture, instance, classe)
	...
	elseif (classe == "PET") then
		...
		-- Grimfall classless compatibility:
		-- always use the dynamically calculated class icon for damage actors.
		texture:SetTexture (instance and instance.row_info.icon_file ~= "" and instance.row_info.icon_file or [[Interface\AddOns\Details\images\classes_small]])
	
		local coords = _detalhes.class_coords [classe] or _detalhes.class_coords ["UNKNOW"]
		texture:SetTexCoord (_unpack (coords))
		texture:SetVertexColor (1, 1, 1)
	end
end
--endd elsefi

function _detalhes:RefreshBarra (esta_barra, instancia, from_resize)

	local class = GetDominantDamageClass(self)
	local enemy, arena_enemy, arena_ally = self.enemy, self.arena_enemy, self.arena_ally

	if (not class) then
		_detalhes:Msg ("Warning, actor without a class:", self.nome, self.flag_original, self.serial)
		class = self.classe or "UNKNOW"
	end
