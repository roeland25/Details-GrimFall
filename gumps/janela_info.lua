...
local elvui_skin = function()
	...
	window.SetClassIcon = function (player, class)
		...
		-- Grimfall compatibility:
		-- classless/custom specs may not exist in Details' spec icon table.
		local specCoords = player.spec and _detalhes.class_specs_coords[player.spec]

		if (specCoords) then
			window.classe_icone:SetTexture ([[Interface\AddOns\Details\images\spec_icons_normal_alpha]])
			window.classe_icone:SetTexCoord (_unpack (specCoords))
